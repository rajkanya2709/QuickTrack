...
Quicktrack is a cross-platform GUI and CLI based tool that helps you to monitor your IP addresses, identify the services running on the open ports as well as help discover any known security vulnerabilities in them. It consumes the <a href="https://internetdb.shodan.io/">InternetDB API</a> of Shodan for the scanning phase.
...
# Setup
In order to run QuickTrack locally on your machine either in CLI or GUI mode, you should firstly meet these requirements:
* ## Golang
...
*  ## Web Server (Only for GUI / Website)
   Next thing you require is a web server software for serving your frontend. For Linux you can check out Apache (sudo apt install apache2) since we used it a **LOT** during the development phase. But for Windows and Mac OS, you can check out <a href="https://www.apachefriends.org/index.html">XAMPP</a>.
...
* ## Email Account
   The next thing you are required to do is setup an email account through which the reports would be sent by the tool. We are using <a href="https://mail.google.com/">Gmail</a> for our website and so far it works perfectly!
...
* ## Compiling
  And finally you are supposed to compile the backend/tool! You can do this by navigating to the <a href="./backend/">backend directory</a> and running:
  ```sh
   go build
  ```
...
QuickTrack takes no responsibility of any intentional misuse of our products and will be willing to share any form of log data if required by the court of law or any law enforcement body.